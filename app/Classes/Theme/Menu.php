...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['separator'])) {
            ...
            {
                // $cls = isset($item['page']) ? $item['page'] : '00.';
                // if(strpos($cls,request()->path())!==false)
                // {
                //     $item_cls = ' menu-item-active';
                // }
                // else
                // {
                //     $item_cls = ' ';
                // }
                // $item_class .= ' '.request()->path();
            }

            
            echo '<li class="menu-item ' . $item_class .$item_cls. '" aria-haspopup="true" ' . $item_attr . '>';
            if (isset($item['parent'])) {
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page']) && strpos($item['page'],$page)!==false) {
            return true;
        }
